...
from brand import (BrandDataModule, BrandDatasets, BrandModel,
                   BrandPredictionWriter)
...
def get_conf_data(train_dataset: BrandDatasets, test_dataset: BrandDatasets, model_arch):
    prediction_root = os.path.join("predictions", 'brand')
    predict_model_name = f"{train_dataset.name}_{model_arch}"
    best_model_path = os.path.join("checkpoints", "brand", f"best_{predict_model_name}.ckpt")
...
def predict_and_persist_brand(
    prediction_root,
    predict_model_name,
    best_model_path,
    prediction_out_file,
    test_dataset:BrandDatasets,
    batch_size,
    allowed_brand_list,
    gpus=None,
    ...
    predict_callback = BrandPredictionWriter(
        prediction_root,
    ...
    model = BrandModel.load_from_checkpoint(best_model_path)
    # init trainer
    trainer = pl.Trainer(
...
def evaluate_predictions(
    ...
    cm = ConfusionMatrixDisplay(cm.numpy(), display_labels=allowed_brand_list)
    cm.plot(
        cmap=plt.cm.Blues,
        values_format=".2f",
    )
